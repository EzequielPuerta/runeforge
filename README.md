...
- **The list is ordered by `attribute` ascending (or `compare`, see below) the whole time reorder is active** — not by `_id`, not by whatever order the backend/array happens to return, and not by clicking a column header either: column-header sorting is unavailable while reorder is active, since row order needs a single, unambiguous source of truth for dragging to mean anything. This holds even if `attribute` is `excludedFromList` and has no column of its own — ordering reads the raw row value regardless of what's rendered.
- **Column filters and reorder share the table, but not at the same time.** Filter controls stay visible the whole time — as soon as any column filter is set, reorder pauses (drag handles disappear, column-header sorting comes back, and the table behaves like a plain filtered/sorted one) since a filtered-out row's drag position would be undefined. Clearing every active filter resumes reorder automatically, back to `attribute`/`compare` ordering.
